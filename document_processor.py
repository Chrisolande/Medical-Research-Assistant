...
class DocumentProcessor:
    ...
    def metadata_func(self, record: Dict[str, Any], metadata: Dict[str, Any]) -> Dict[str, Any]:
        for field in self.metadata_fields:
            if field in record:
                metadata[field] = str(record[field]) if record[field] is not None else ""
    ...
    def _validate_and_clean(self, file_path: str, text: str) -> tuple[Path, str]:
        ...
        return path, re.sub(r'\s+', ' ', re.sub(r'([.!?])\s*([A-Z])', r'\1 \2', text.strip()))


    def load_and_process_documents(self, file_path: str, content_key: str = "abstract",
                             jq_schema: str = ".[]", max_docs: Optional[int] = None,
                             min_chunk_size: int = 50) -> List[Document]:
        try:
            validated_path, _ = self._validate_and_clean(file_path, "")

            loader = JSONLoader(str(validated_path), jq_schema, content_key, self.metadata_func)
            ...
            logger.info(f"Loaded and pre-processed {len(valid_docs)} documents from {file_path}")
            return valid_docs
    ...
    def process_documents(self, documents: List[Document], min_chunk_size: int = 50) -> List[Document]:
        """
        ...
        final_chunks = [doc for doc in chunks if len(doc.page_content.strip()) >= min_chunk_size]
    ...
    def get_stats(self, documents: List[Document]) -> Dict[str, Any]:
        if not documents: return {"total_chunks": 0, "total_characters": 0, "average_chunk_size": 0}
    ...
    def save_processed_documents(self, documents: List[Document], output_path: str) -> None:
        data = {
            "processing_info": {"embeddings_model": self.embeddings_model, "total_chunks": len(documents)},
            "documents": [{"chunk_id": i, "content": doc.page_content, "metadata": doc.metadata}
