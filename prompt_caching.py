...
from config import (
    DUMMY_DOC_CONTENT, DEFAULT_DATABASE_PATH, DEFAULT_FAISS_INDEX_PATH,
    DEFAULT_SIMILARITY_THRESHOLD, DEFAULT_MAX_CACHE_SIZE,
    DEFAULT_MEMORY_CACHE_SIZE, DEFAULT_BATCH_SIZE, ENABLE_QUANTIZATION,
    EMBEDDING_MODEL_NAME
)
from utils import run_in_executor, log_info, log_error 
...
class SemanticCache(SQLiteCache):
    ...
    def _init_semantic_store(self):
        if os.path.exists(self.faiss_index_path) and os.path.isdir(self.faiss_index_path):
            ...
            log_info(f"FAISS index path {self.faiss_index_path} not found or not a directory. Creating a new index.")
            self._create_new_faiss_index()

    async def _init_semantic_store_async(self):
        if os.path.exists(self.faiss_index_path) and os.path.isdir(self.faiss_index_path):
            try:
                self.vector_store = await run_in_executor(self.executor, self._load_faiss_index) # No parentheses for func
    ...
    def _create_new_faiss_index(self):
        try:
            ...
            log_error(f"Error: Failed to initialize FAISS vector store: {e}", exc_info=True)
            self.vector_store = None

    async def _create_new_faiss_index_async(self):
        try:
            self.vector_store = await run_in_executor(self.executor, self._create_faiss_from_texts, [DUMMY_DOC_CONTENT], [{"type": "initializer", "is_dummy": True}])
            log_info("Created new FAISS index successfully")

        except Exception as e:
            log_error(f"Error: Failed to initialize FAISS vector store: {str(e)}", exc_info=True)
            self.vector_store = None
    ...
    def lookup(self, prompt: str, llm_string: str):
        ...
        if self.vector_store is None:
            log_info("Semantic vector store is not initialized. Skipping semantic search")
            self.metrics["cache_misses"] += 1
            return None

        try:
            if self._is_dummy_only():
                log_info("The vector store contains only the initializer document. Skipping semantic search")
                self.metrics["cache_misses"] += 1
                return None
            ...
            if docs_with_score:
                for doc, score in docs_with_score:
                    ...
                    if score <= self.similarity_threshold: # Less is better
                        ...
                        log_info(f"Best match score {score:.4f} above threshold ({self.similarity_threshold}).")
                        break
    ...
    def _is_dummy_only(self):
        if len(self.vector_store.index_to_docstore_id) <= 1:
            for doc_id in self.vector_store.index_to_docstore_id.values():
                ...
                if doc and doc.page_content == DUMMY_DOC_CONTENT and doc.metadata.get("is_dummy"):
                    return True
    ...
    def update(self, prompt: str, llm_string: str, return_val: List[Generation]):
        ...
        if self.vector_store is None:
            log_info("Semantic vector store not initialized, attempting to re-initialize.")
            self._init_semantic_store()
            if self.vector_store is None:
    ...
    async def update_async(self, prompt: str, llm_string: str, return_val: List[Generation]):
        ...
        try:
            ...
            await run_in_executor(self.executor, self._add_to_vector_store, prompt, metadata)
    ...
    def _remove_dummy_doc(self):
        dummy_ids = [doc_id for doc_id in self.vector_store.docstore._dict.keys()
                     if self.vector_store.docstore._dict.get(doc_id) and
                        self.vector_store.docstore._dict.get(doc_id).page_content == DUMMY_DOC_CONTENT]
        if dummy_ids:
