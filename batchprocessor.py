...
from config import (
    PMC_BATCH_SIZE, PMC_MAX_CONCURRENT_BATCHES, PMC_RETRY_ATTEMPTS,
    PMC_RETRY_DELAY, PMC_INTER_BATCH_DELAY, MIN_ABSTRACT_CONTENT_LENGTH
)
from utils import load_json_data, create_batches, save_processing_results 
...
class PMCBatchProcessor:
    ...
    def load_pmc_data(self, file_path:str, max_docs: Optional[int] = None) -> List[Dict[str, Any]]:
        pmc_docs = load_json_data(file_path, max_docs)

        # Make sure that the documents(all of them have abstracts)
        valid_docs = [doc for doc in pmc_docs if doc.get('abstract', '').strip() and len(doc.get('abstract', '').strip()) >= MIN_ABSTRACT_CONTENT_LENGTH]

        if len(pmc_docs) != len(valid_docs):
            self.logger.info(f"Filtered {len(pmc_docs) - len(valid_docs)} documents lacking valid abstracts or too short.")
    ...
    async def _process_batch_async(self, batch: List[Dict[str, Any]], batch_num: int) -> Dict[str, Any]:
        for attempt in range(self.retry_attempts):
            try:
                self.logger.info(f"Processing batch {batch_num} "
                                 f"({len(batch)} documents, attempt {attempt + 1})")

                documents = await asyncio.get_event_loop().run_in_executor(self.executor, self._process_batch_documents, batch)
                return {
                    "batch_num": batch_num,
                    "success": True,
                    "documents": documents,
                ...
                self.logger.warning(f"Batch {batch_num} attempt {attempt + 1} failed: {str(e)}", exc_info=True)
    ...
    async def process_pmc_file_async(
        ...
        results["processing_summary"] = {
            ...
            "success_rate": len(results["successful_batches"]) / total_batches * 100 if total_batches > 0 else 0.0,
            "processing_time": processing_time,
            "avg_time_per_batch": processing_time / total_batches if total_batches > 0 else 0.0,
